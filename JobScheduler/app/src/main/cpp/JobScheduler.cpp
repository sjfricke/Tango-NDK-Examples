...
namespace job {
  ...
  void JobScheduler::OnCreate(JNIEnv* env, jobject caller_activity)
  {
    ...
    if (TANGO_SUCCESS != err || version < kTangoCoreMinimumVersion) {
      LOGE("JobScheduler::CheckVersion, Tango Core version is out of date.");
  ...
  void JobScheduler::OnTangoServiceConnected(JNIEnv* env, jobject iBinder)
  {
    ...
    if (TANGO_SUCCESS != err) {
      LOGE("JobScheduler::ConnectTango ,config_enable_depth() failed with error code: %d.", err);
      std::exit(EXIT_SUCCESS);
    }
    ...
    if (TANGO_SUCCESS != TangoService_connect(this, tango_config_)) {
      LOGE("JobScheduler::ConnectTango, TangoService_connect error.");
      std::exit(EXIT_SUCCESS);
    }
